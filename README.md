...
**[Background]** Time-space traffic diagrams (or spatiotemporal speed contour diagrams) visualize how traffic speed changes over time and space. They are commonly constructed by averaging speed observations within **rectangular cells**.
...
**[Contribution]** This project implements a simple method that aggregates speed observations within non-rectangular parallelogram cells aligned with backward-moving traffic waves. A Python tool supports both rectangular and parallelogram cells.

**[Impact]** The method provides a practical way to construct time-space traffic diagram from trajectory or probe data using only time, location, and speed, without requiring vehicle identities. The following paper reports improvements particularly under congested conditions and with relatively large cells.
...
To use your own data, replace the CSV path. By default, the tool generates both diagrams with the following changeable parameters (a **30 s** cell width, **50 m** cell height, **−16 km/h** wave speed), and no empty-cell filling. Use `--dt`, `--dx`, and `--wave-speed` to change these settings; `--mode rectangular` or `--mode parallelogram` selects one method. Run `tx-diagram --help` for all options.
